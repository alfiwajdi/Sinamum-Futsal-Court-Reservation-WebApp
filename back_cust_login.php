<?php
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        try{
        // Prepare statement
        $stmt = mysqli_prepare($mysqli, "SELECT password FROM customer WHERE cust_id = ?");
    ...
    function success_login(){
        header("location: cust_homepage.php?login=TRUE");
    }

    //if login invalid function
    function invalid_login() {
        header("location: cust_login.php?login=FALSE");
      }
?>